...
short * ForwardBWT(BWC & blck)
{
    ...
    if (!blck.aml)
        blck.aml = calculate_aml(suffixes, blck.input_len);
...
short * InverseBWT(short * bwt_seq, unsigned Len, unsigned index, short offset)
{
    ...
    while (k--)
    {
        if ((I = P[I] + C[tmp = bwt_seq[I]]) > index)
            --I;
...
int16_t ** med3func(int16_t ** a, int16_t ** b, int16_t ** c, int depth)
{
    int va, vb, vc;
...
void ssort2(int16_t ** a, int n, int depth)
{
    int d, r, partval;
    int16_t **pa, **pb, **pc, **pd, **pl, **pm, **pn, *t;
    if (n < 10)
    {
        inssort(a, n, depth);
        return;
