...
ECGSignalSPtr ECGSignal::fromDaqViewFile(const std::string &ecgFileName)
{
    auto instance = std::make_shared<ECGSignal>();
    ...
    try
    {
        ...
        while (io$.good())
        {
            ...
            if (line == "*CHANNELS")
            {
                while (io$.good())
                {
                    ...
                    char chName[12];
                    char chUnits[12];
                    sscanf(line.c_str(), "%d,%lf,%s,%s", &chNum, &resolution_uV, chName, chUnits);
        ...
        auto numValues = static_cast<size_t>(byteLength) / sizeof(int16_t);
